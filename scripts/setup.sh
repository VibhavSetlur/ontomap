...
echo "  python scripts/download_models.py          # SapBERT + MedCPT encoders"
echo "  python scripts/build_corpus.py --patches   # ModelSEED corpus for the reaction pipeline"
echo "  python scripts/link_lora_adapters.py       # LoRA adapters (if in a sibling workspace)"
echo "  python scripts/regen_embeddings.py         # cache SapBERT corpus embeddings (~30s on GPU, self-checks)"
echo "  ontomap info                               # health-check every asset, then:"
