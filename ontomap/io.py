...
def _introspect_sqlite(db_path: Path) -> tuple[list, list, dict]:
    """Return (tables, views, row_counts) from a live SQLite DB.

    tables/views are [(name, [(col, type, pk_bool), ...]), ...] in name order.
    Uses the sqlite_master + PRAGMA table_info pattern (same as
    examples/05_sqlite_output.py).
    """
    conn = sqlite3.connect(str(db_path))
    try:
        tables, views, counts = [], [], {}
        for typ, bucket in (("table", tables), ("view", views)):
            names = [
                row[0]
            ...
            for name in names:
                ...
                try:
                    ...
                    counts[name] = None
        return tables, views, counts
    finally:
        conn.close()


def write_sqlite_readme(
    db_path: Path,
    *,
    ...
    """Write a self-documenting README.md beside a SQLite deliverable.

    Introspects the DB's live schema (tables, views, columns) and emits file
    inventory + headline counts + full schema + copy-paste SQL + (for annotated
    DBs) the MSRXN: join gotcha. Best-effort: returns the README path on
    success, None on failure (never raises — a doc hiccup must not fail a build).

    kind: "annotated" (reaction-pipeline deliverable), "model" (modelmap
    compound/reaction), or "auto" (detect from table names).
    """
    try:
        db_path = Path(db_path)
        tables, views, live_counts = _introspect_sqlite(db_path)
        tnames = {t[0] for t in tables}
        if kind == "auto":
            if "existing_reactions" in tnames or "annotations" in tnames:
                kind = "annotated"
            elif "compound_predictions" in tnames or "compounds" in tnames:
                kind = "model"
            else:
                kind = "core"

        # Prefer a sibling README; fall back to <stem>.README.md if one exists.
        readme = db_path.parent / "README.md"
        if readme.exists():
            readme = db_path.with_suffix(db_path.suffix + ".README.md")

        c = counts or live_counts
        lines: list[str] = []
        title = {
            "annotated": "ontomap reaction predictions",
            "model": "ontomap model → ModelSEED mapping",
            "core": "ontomap predictions",
        }[kind]
        lines.append(f"# {title} — `{db_path.name}`")
        lines.append("")
        lines.append(
            "Auto-generated by ontomap when the SQLite was written. The database "
            "is self-contained — everything below was introspected from it, so "
            "this doc cannot drift from the data."
        )
        if pipeline_version:
            lines.append("")
        ...
        if c:
            ...
            for k in sorted(c):
                v = c[k]
                lines.append(f"| `{k}` | {v:,} |" if isinstance(v, int) else f"| `{k}` | — |")

        # Schema
        def _emit(objs, header):
            if not objs:
                return
            lines.append("")
            lines.append(f"## {header}")
            for name, cols in objs:
                n = live_counts.get(name)
                rowtxt = f" — {n:,} rows" if isinstance(n, int) else ""
                lines.append("")
                lines.append(f"### `{name}`{rowtxt}")
                lines.append("")
                lines.append("| column | type | pk |")
                lines.append("|---|---|:--:|")
                for col, ctype, pk in cols:
                    lines.append(f"| `{col}` | {ctype or '—'} | {'✓' if pk else ''} |")

        _emit(tables, "Tables")
        _emit(views, "Views")

        # Quick SQL + gotchas, tailored by kind
        lines.append("")
        lines.append("## Quick SQL")
        lines.append("")
        if kind == "annotated":
            lines.append("```sql")
            lines.append("-- Top-1 prediction per description")
            lines.append("SELECT description_id, description, reaction_id, fused_score,")
            lines.append("       predicate, reaction_name")
            lines.append("FROM prediction_full WHERE rank = 1;")
            lines.append("")
            lines.append("-- Full top-10 for one description")
            lines.append("SELECT rank, reaction_id, fused_score, predicate, reaction_name")
            lines.append("FROM prediction_full")
            lines.append("WHERE description_id = '<DESC|...>' AND rank <= 10 ORDER BY rank;")
            lines.append("```")
            lines.append("")
            lines.append(
                "> **Join gotcha.** `existing_reactions.reaction_id` carries the "
                "`MSRXN:` prefix (e.g. `MSRXN:rxn02167`) while "
                "`predictions.reaction_id` is the bare id (`rxn02167`). Strip the "
                "prefix when joining: "
                "`REPLACE(er.reaction_id,'MSRXN:','') = p.reaction_id`."
            )
        elif kind == "model":
            lines.append("```sql")
            lines.append("SELECT * FROM compound_top_n WHERE rank = 1 LIMIT 5;")
            lines.append("SELECT * FROM reaction_top_n WHERE rank = 1 LIMIT 5;")
            lines.append("```")
        else:
            lines.append("```sql")
            lines.append("SELECT * FROM top_n_with_meta WHERE rank = 1 LIMIT 5;")
            lines.append("```")

        lines.append("")
        lines.append(
            "Regenerate this doc for any ontomap SQLite with "
            "`ontomap describe <file.sqlite>`."
        )
        readme.write_text("\n".join(lines) + "\n")
        return readme
    except Exception as e:  # never fail a build over docs
        try:
            import logging

            logging.getLogger("ontomap.io").warning(
                "schema README generation skipped: %s", e
            )
        except Exception:
            pass
        return None
...
def write_sqlite(results, path: Path, pipeline_version: str = "pipeline_3-v0.1.0",
    ...
    try:
        ...
        conn.close()
    if _emit_readme:
        write_sqlite_readme(path, pipeline_version=pipeline_version, kind="core")
