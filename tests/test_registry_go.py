...
def test_go_text_is_deterministic_and_batched_contract():
    ...
    assert one.provenance["artifact_version"] == "filipe-go-height-lte-1-dedup-v1"
...
def test_corrected_height_lte_go_version_is_default_and_prior_versions_roll_back():
    registry = default_registry()
    assert registry.resolve("go-text").version == "4.0.0"
    assert registry.resolve("go-text", "1.0.0").version == "1.0.0"
    assert registry.resolve("go-text", "4.0.0").plugin.metadata["default"] is True
    assert registry.resolve("go-text", "3.0.0").plugin.metadata["default"] is False
...
def test_corrected_height_lte_go_default_maps_arbitrary_text_with_versioned_provenance():
    result = map_text("arbitrary future enzyme text with no identifier", top_k=2)
    assert len(result.predictions) == 2
    assert result.provenance["method_version"] == "4.0.0"
    assert result.provenance["artifact_version"] == "filipe-go-height-lte-1-dedup-v1"
    assert result.provenance["training"]["train_entities"] == 94675


def test_height_lte_dedup_artifact_is_preparation_only_not_runtime_filter():
    manifest = verify_manifest(Path(__file__).parents[1] / "ontomap/artifacts/filipe-go-height-lte-1-dedup-v1/manifest.json")
    assert manifest["filter"]["id"] == "go-height-lte-1-v1"
    assert manifest["filter"]["rule"] == "retain GO terms with supplied ontology height <= 1; exclude height >= 2"
    assert manifest["filter"]["stage"] == "pre-training only; runtime predictions are never height-filtered"
    assert manifest["preparation"]["dedup_components"] == 10009
    assert manifest["metric"]["candidate_labels"] == 3979
    assert manifest["promotion"]["previous_default_method_version"] == "3.0.0"
    assert manifest["compatibility"]["rollback_method_versions"] == ["2.0.0", "3.0.0"]
